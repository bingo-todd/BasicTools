...
def parse_args():
    ...
    parser.add_argument('--wav', dest='wav_path', required=True, nargs='+',
            type=str, help='')
    parser.add_argument('--frame-len', dest='frame_len', type=float, 
            default=-1, help='frame length in s')
    parser.add_argument('--frame-shiftt', dest='frame_shift', type=float, 
            default=None, help='frame length in s')
...
def main():
    ...
    tar, fs_tar = wav_tools.read_wav(tar_path)

    if len(args.wav_path) > 1:
        inter_path = args.wav_path[1]
        inter, fs_inter = wav_tools.read_wav(inter_path)
        if fs_tar != fs_inter:
    ...
    snrs = wav_tools.cal_snr(tar=tar, inter=inter, frame_len=frame_len, frame_shift=frame_shift)
